...
###################
    ...
    def search_by_album(self, search_string):
        # sql call construction
        query = """SELECT al.title, ar.name, al.year, al.id
                   FROM album AS al, artist AS ar
                   WHERE lower(al.title) LIKE lower(%s)
                   AND al.artist_id = ar.id
                   ORDER BY al.title, ar.name, al.year"""

        # add % to the string for LIKE
        search_string = '%' + search_string + '%'
    ...
    def get_artists(self):
        ...
        try:
            ...
            # get rid of brackets around name
            resultset = list(itertools.chain.from_iterable(resultset))
            return resultset
